...
def build_header():
    ...
    st.write(f'''<p style='text-align: center'>
            Bem-vindo à nossa ferramenta interativa que permite simular o jogo de acordo com diversas características usando o modelo Random Forest. Explore as opções abaixo para personalizar sua simulação.</p>
            ''', unsafe_allow_html=True)
    st.markdown("---")
...
def keyword_extraction_and_word_cloud(filtered_data, df_reviews, variavel_gambiarra):
    ...
    try:
        ...
        filtered_reviews = classificadores.random_forest(df_reviews[df_reviews["app_id"].isin(selected_games)])[6]
        ...
        positive_cloud = WordCloud(width=1000, height=600, background_color="black",
                                   max_words=25).generate_from_frequencies(positive_freq_dict)
        negative_cloud = WordCloud(width=1000, height=600, background_color="black",
                                   max_words=25).generate_from_frequencies(negative_freq_dict)
